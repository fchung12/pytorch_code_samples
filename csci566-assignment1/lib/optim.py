...
class Adam(Optimizer):
    ...
    def update(self, layer):
        ...
        for n, dv in layer.grads.items():
            ...
            # initailize mt and vt to 0 : piazza@91_f45
            try:
                self.mt[n]
                self.vt[n]
            except KeyError:
            ...
            m_hat = self.mt[n]/(1-(self.beta1**self.t))
            v_hat = self.vt[n]/(1-(self.beta2**self.t))
            
            layer.params[n] = layer.params[n] - ((self.lr*m_hat)/(np.sqrt(v_hat) + self.eps))
